...
class Main_menu(UI):
    ...
    async def move_ball(self):
        ...
        if self.ball_pos_y >= self.board_end_y:
            ...
            if (
                ...
                self.window.addstr(
                    self.board_end_y, self.board_end_x, self.bottom_right_corner, curses.color_pair(9)
                )
            else:
                self.window.addstr(
                    self.board_end_y, self.ball_pos_x, self.horizontal_border, curses.color_pair(9)
                )
        ...
        if self.ball_pos_y <= self.board_start_y:
            ...
            self.window.addstr(self.board_start_y, self.ball_pos_x,self.horizontal_border, curses.color_pair(9))
            self.window.addstr(
                self.board_end_y, self.ball_pos_x, self.horizontal_border, curses.color_pair(9)
            )
        ...
        if self.ball_pos_x >= self.board_end_x:
            ...
            self.window.addstr(self.ball_pos_y, self.board_end_x, self.vertical_border, curses.color_pair(9))
        ...
        if self.ball_pos_x <= self.board_start_x:
            ...
            self.window.addstr(
                self.ball_pos_y, self.board_start_x, self.vertical_border, curses.color_pair(9)
            )
    ...
    async def view(self, app):
        ...
        for x in range(self.board_start_x, self.board_end_x):
            self.window.addstr(self.board_start_y, x, self.horizontal_border, curses.color_pair(1))

        for y in range(self.board_start_y + 1, self.board_end_y):
            self.window.addstr(y, self.board_start_x, self.vertical_border, curses.color_pair(1))

        self.window.addstr(
            self.board_start_y, self.board_start_x, self.upper_left_corner, curses.color_pair(1)
        )
        self.window.addstr(
            self.board_start_y, self.board_end_x, self.upper_right_corner, curses.color_pair(1)
        )

        for y in range(self.board_start_y + 1, self.board_end_y):
            self.window.addstr(y, self.board_end_x, self.vertical_border, curses.color_pair(1))

        self.window.addstr(
            self.board_end_y, self.board_start_x, self.bottom_left_corner, curses.color_pair(1)
        )
        self.window.addstr(self.board_end_y, self.board_end_x, self.bottom_right_corner), curses.color_pair(1)

        for x in range(self.board_start_x + 1, self.board_end_x):
            self.window.addstr(self.board_end_y, x, self.horizontal_border, curses.color_pair(1))
